...
- 구현 완료 후 여유가 생긴다면, Step 2를 미리 준비하기보다 팀원 1의 `attention.py` 구현을 함께 검토하고 디버깅을 도와주는 것을 권장합니다. 팀원 1이 빠르게 완료되어야 `sanity_check.py` 실행이 가능하고, 이는 팀원 2 본인의 코드 통합 테스트에도 직접적인 영향을 줍니다.

**팀원 3** (`optimizer.py` + `classifier.py`)
