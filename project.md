...
| 파일 | 구현 내용 | 담당 |
|------|----------|------|
| `modules/attention.py` | `attention()` | 팀원 1 |
| `modules/gpt2_layer.py` | `add()`, `forward()` | 팀원 2 |
| `models/gpt2.py` | `embed()`, `hidden_state_to_token()` | 팀원 2 |
| `optimizer.py` | `AdamW.step()` | 팀원 3 |
| `classifier.py` | `GPT2SentimentClassifier.__init__()`, `forward()` | 팀원 3 |

### 역할별 장단점

**팀원 1: 박서연님** (`attention.py`)
- 장점: 수업의 핵심 개념인 Attention을 가장 깊이 이해하게 됨. 구현 하나에 집중할 수 있어 코드 품질을 높이기 좋음
- 단점: 난이도가 가장 높아 막히면 전체 일정에 영향을 줄 수 있음

**팀원 2: XIONG MINGXING님** (`gpt2_layer.py` + `gpt2.py`)
- 장점: GPT-2 전체 구조를 가장 폭넓게 이해하게 됨. 구현할 함수가 4개(`add()`, `forward()`, `embed()`, `hidden_state_to_token()`)로 셋 중 가장 많음
- 단점: `gpt2_layer.py`는 팀원 1 코드와 연결되어 통합 테스트 시 의존성이 생김. 구현 분량이 가장 많아 시간이 오래 걸릴 수 있음

**팀원 3: 이서진님** (`optimizer.py` + `classifier.py`)
- 장점: `optimizer_test.py`로 팀원 1+2 완료 전에 혼자 검증 가능. `classifier.py`를 통해 fine-tuning 실험을 직접 돌려보는 경험을 얻을 수 있음
- 단점: `classifier.py` 학습 실행이 팀원 1+2 완료에 달려있어 대기 시간이 생길 수 있음
...
알겠어. 아래 내용으로 PROJECT.md 협업 규칙 섹션을 교체하면 돼.
...
| 날짜 | 할 일 | 담당 |
|------|-------|------|
| ~ 5/13 24:00 | `attention()` 구현 완료 | 팀원 1 |
| ~ 5/16 24:00 | `add()`, `forward()`, `embed()`, `hidden_state_to_token()` 구현 완료 | 팀원 2 |
