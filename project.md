...
| 파일 | 구현 내용 | 담당 |
|------|----------|------|
| `modules/attention.py` | Masked Multi-Head Self-Attention | 팀원 1 |
| `modules/gpt2_layer.py` | GPT2Layer.add(), forward() | 팀원 2 |
| `models/gpt2.py` | embed() | 팀원 2 |
| `optimizer.py` | AdamW.step() | 팀원 3 |
| `classifier.py` | GPT2SentimentClassifier | 팀원 3 |
...
- 장점: GPT-2 전체 구조를 가장 폭넓게 이해하게 됨. `gpt2.py`의 `embed()`는 비교적 단순해서 부담이 적음
- 단점: 파일 수가 2개로 가장 작업량이 적을 수 있음. `gpt2_layer.py`는 팀원 1 코드와 연결되어 통합 테스트 시 의존성이 생김
- gpt2_layer.py 구현 완료 후 여유가 생긴다면, Step 2를 미리 준비하기보다 팀원 1의 attention.py 구현을 함께 검토하고 디버깅을 도와주는 것을 권장합니다. 팀원 1이 빠르게 완료되어야 sanity_check.py 실행이 가능하고, 이는 팀원 2 본인의 코드 통합 테스트에도 직접적인 영향을 줍니다.
