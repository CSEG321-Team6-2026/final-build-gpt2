...
| `optimizer.py` | AdamW.step() | 중간 | 팀원 2 |
| `models/gpt2.py` | embed() | 낮음 | 팀원 3 |
...
**역할 분담**
- 팀원 1: `attention.py` 구현
- 팀원 2: `gpt2_layer.py` + `optimizer.py` 구현
- 팀원 3: `gpt2.py` + `classifier.py` 구현
...
2. **`optimizer_test.py`** (로컬, 팀원 2 완료 후)
Adam Optimizer 구현 검증. 팀원 2 혼자 실행 가능합니다.

3. **`sanity_check.py`** (로컬, 팀원 1 + 2 + 3 완료 후)
GPT-2 구현 검증. 팀원 1, 2, 3의 구현이 모두 완료되어야 실행 가능합니다.
